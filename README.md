...
Docker 默认将 `10000` 端口发布到 `0.0.0.0`，可供局域网、FRP 或反向代理访问。生产环境必须使用 HTTPS，并在 `.env` 中设置 `QUANTPILOT_COOKIE_SECURE=true`。网页中更新 Alpaca 配置后，该用户的交易引擎会进入安全暂停状态，需在“自动交易”页面确认后重新启动。管理员网页配置优先于 `.env`；普通用户没有共享的 `.env` 后备凭据。

也可以直接拉取 AMD64/ARM64 公共镜像：`ghcr.io/charles-0509/quantpilot:1.3.0` 或 `ghcr.io/charles-0509/quantpilot:latest`。

已有部署在线升级（Alembic 会自动保留数据并把原有记录归到管理员）：

```bash
cd /opt/quantpilot
sudo docker compose -p quantpilot -f compose.quantpilot.yml --env-file .env pull
sudo docker compose -p quantpilot -f compose.quantpilot.yml --env-file .env up -d --pull always
sudo docker compose -p quantpilot -f compose.quantpilot.yml --env-file .env logs -f --tail=100
```
