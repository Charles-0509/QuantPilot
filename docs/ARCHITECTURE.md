...
内部时间继续以 UTC 保存和计算，所有 Web UI 文本、回测时间和行情图表坐标统一按 `Asia/Shanghai`（UTC+8）格式化。1小时和日线聚合使用 Alpaca 官方交易日历处理13:00提前收市。策略成交会写入独立的策略持仓归属账本；普通离场只能卖出该策略拥有的数量。未识别的手动持仓不会归属策略；检测到手工卖出、实际持仓不足、开放卖单超量或空头时，系统持久化执行安全隔离、暂停引擎、取消 QuantPilot 自有订单并清除不再可信的策略归属，优先避免误平仓或建立空头。
